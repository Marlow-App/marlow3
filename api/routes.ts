...
import { generatePhraseAudio, getPhraseAudioFile } from "./elevenlabs.js";
import { countChineseChars, MAX_CHARS, FREE_RECORDINGS_PER_DAY, FREE_PRACTICE_LIST_MAX, SUBSCRIPTION_PLANS } from "../shared/credits.js";
import { sendFeedbackNotification, sendRecordingNotification, sendSupportEmail } from "./email.js";
import { scoreMandarin } from "./speechsuper.js";
  ...
  app.post(api.feedback.create.path, isAuthenticated, async (req, res) => {
    try {
      ...
      if (characterRatings && Array.isArray(characterRatings) && characterRatings.length > 0) {
        const { characterRatingSchema } = await import("../shared/schema.js");
